...
target_dir = gf.DATA_DIR + '\Form Submissions'
output_dir = gf.OUTPUT_DIR + '\User Activity'


# if only want to test with a subset of dates:
start_date = pd.Timestamp('03-20-2017')
end_date = pd.Timestamp('07-02-2018')
...
# how long is considered to learn the app in days
# note that per calendar, 'start' included - so is this period minus 1 day
startup_period = 60
...
if download_new is True:
    file_list = ff.download_form_ucr(download_start, download_stop, gf.credential_path,
...
forms_df['active_date'] = forms_df['start_date'].apply(lambda x: str((pd.to_datetime(x) + np.timedelta64(startup_period, 'D')).date()))
...
forms_df = forms_df.assign(days_since_active= lambda x: ((last_date - pd.to_datetime(x.active_date)) / np.timedelta64(1, 'D')) + 1)

# reorder columns
column_order = (locations + \
                ['start_date', 'last_submission', 'active_date', 'days_inactive', 'days_since_start', 'days_since_active'] + \
                date_cols)
...
tallies_df = forms_df.filter(['block_name',
                              ...
                              'last_submission', 'start_date', 'active_date',
                              'days_inactive', 'days_since_start', 'days_since_active'])
...
for location in locations:
    loc = locations[count:]
    new_agg = tallies_df.groupby(loc)['active_date'].min()
    new_agg.to_csv('active_date_by_%s.csv' % (loc[0][:-5]))
    count = count + 1
...
logging.info('assigning pre dates...')
temp_forms_np[activity_df['start_date'].values[:, None] > activity_df.columns[non_date_cols:].values] = "pre"

# assign 'training' if start_date < col_date < start_date + startup_period
logging.info('assigning training dates...')
temp_forms_np[(activity_df['start_date'].values[:, None] < activity_df.columns[non_date_cols:].values) & (activity_df['active_date'].values[:, None] > activity_df.columns[non_date_cols:].values)] = "training"
...
j = 'inactive_' + i.apply(_assign_inactive, axis=1).astype(str)
...
activity_df[date_cols] = activity_df[date_cols].apply(np.vectorize(_inactive_group))
...
row_order = ['pre', 'start', 'training', 'active', 'inactive_1-3', 'inactive_4-7',
